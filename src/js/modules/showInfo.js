    ...
    this.btns.forEach(btn => {
      btn.addEventListener('click', () => {
        if (btn.closest('.module__info-show') && btn.closest('.module__info-show').nextElementSibling.style.display !== 'block') {
          ...
          try { btn.closest('.module__info-show').nextElementSibling.style.display = 'block'; } catch (error) { }
        } else {
          btn.parentNode.style.cssText = `transform: rotate(0deg); transition: all .3s`;
          try { btn.closest('.module__info-show').nextElementSibling.style.display = 'none'; } catch (error) { }
        }
      });
    });
  }
